...
# --no-deps: dependencies are handled by requirements-colab.txt above
# (pyproject.toml uses floor pins so a plain install would also work, but
# --no-deps keeps pip fast and idempotent on Colab).
